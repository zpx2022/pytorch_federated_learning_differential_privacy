...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **FedAvg** | 0.9858 (919轮) | 0.9828 (828轮) | 0.9785 (576轮) | 0.9761 (532轮) | 0.9769 (410轮) | 0.9715 (363轮) | 0.9735 (432轮) |
...
3.  **LDP 环境下的合适噪声强度**：基于预设的效用目标 即最高准确率（Max Accuracy） 损失小于0.01，FedAvg算法采用0.05、0.06、0.07、0.08，FedProx算法采用0.05、0.06、0.07、0.08的laplace_noise_scale，FedNova算法采用0.05、0.06的laplace_noise_scale较为合适   

4.  **最终结论**：本次系列实验表明，在为联邦学习系统设计隐私保护机制时，**不能孤立地看待联邦学习算法和隐私技术**。
...
为了验证该假设，本人设计了补充实验，将学习率降低为 `0.005` 并重新运行。实验结果完美地证实了我们的猜想：
![](figures/SCAFFOLD_lr_0.005.png)

  * **稳定的新基准**：在 `lr=0.005` 时，无噪声的基准实验变得非常稳定，并成功收敛至 **96.96%** 的最高准确率。
  * **真实的权衡**：在这个稳定基线上，加入了LDP噪声（强度0.05）的版本准确率则为 **97.53%**。
